...
- 多模态开关：视觉模型（Qwen2.5-VL / LLaVA / MiniCPM-V 等）勾选后启动带 `--mmproj`，自动加载 `models/` 里的 mmproj 投影文件
...
- **多模态**：勾选参数区的「多模态」→ 启动带 `--mmproj`，自动在 `models/` 目录匹配 mmproj 投影文件（找不到会在日志提示）；作为预设参数保存，默认关
